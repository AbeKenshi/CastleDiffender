...
public:
	...
	// 一番近くのバリケードの位置を返す関数
	VECTOR2 searchNearBarricade(Entity &entity, Barricade *barricades);
	// 一番近くのバリケードのインデックスを返す関数
	int searchNearBarricadeIndex(Barricade *barricades);
	// 雑魚敵からある点までの直線上にバリケードが存在したらtrue、存在しない場合はfalseを返す。
	bool checkBarricadeOnLine(float x, float y, Barricade *barricades);
